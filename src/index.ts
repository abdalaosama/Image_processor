...
app.get('/serve/:filename', (req: express.Request, resp: express.Response) => {// server image file
  try {
    ...
    Log(`filename: ${filename}, width: ${width}, height: ${height}, from IP: ${req.ip}`);
    // make sure those inputs are safe (unpolluted)
    let imagePath: string = path.join(config.assetsFolder, '/thump', `${filename.split('.')[0]}${width}x${height}.${filename.split('.')[1]}`);
    ...
    if (!imageExists) {
        ...
        const originalImagePath: string = path.join(config.assetsFolder, '/full', filename);
